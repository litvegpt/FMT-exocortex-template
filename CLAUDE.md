...
4. **Pull-on-Touch:** `git pull --rebase` при первом обращении к репо за сессию (все `/Users/evgeny/Github/*`). Dirty → stash; конфликт → `memory/reference/agent-core.md`.
...
Рабочая директория: `/Users/evgeny/IWE/`; `memory/` = симлинк на auto-memory.
...
Hot-каркас ≤20K токенов (M1), строгая цель ≤12K (M2). Изменил файл из `hot-files.list` (оба CLAUDE.md, rules/*.md) → перед коммитом `/Users/evgeny/Github/FMT-exocortex-template/scripts/verify-context-budget.sh`.
...
- **Комментарии кода — только EN (решение Андрея, 14.06.2026):** весь `/Users/evgeny/Github/**`; исключение — user-facing строки по языку интерфейса.
...
- **Именование:** governance-репо называется `DS-strategy` по умолчанию; рабочая директория `/Users/evgeny/IWE/`.
