...
|-----|-----------|---------|---------|
| **Системы** | U.System | Claude Code, бот, MCP, WakaTime, launchd, Git | Нет исполнения |
| **Знания** | U.Episteme | Экзокортекс (CLAUDE.md + memory/), Pack, промпты, методология (FPF/SPF) | ИИ = stateless, вайб-работа |
...
> **Почему 4 вида, а не плоский список?** IWE содержит элементы разных онтологических типов. Системы запускаются, знания загружаются, роли назначаются, артефакты производятся. Плоский список смешивает типы — категориальная ошибка (FPF A.7 Strict Distinction).

> **Почему IWE, а не «экзокортекс»?** Экзокортекс — знания и инструкции (Вид 2: Знания), один из четырёх видов. Называть всю среду «экзокортексом» = называть компьютер «жёстким диском». Подробнее: [DP.IWE.001 §2.1](https://github.com/TserenTserenov/PACK-digital-platform/blob/main/pack/digital-platform/02-domain-entities/DP.IWE.001-intelligent-working-environment.md).
