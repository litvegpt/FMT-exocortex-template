...
if $DRY_RUN; then
    ...
    echo "    дlitvegpt → $GITHUB_USER"
    echo "    /home/evgeny/Github → $WORKSPACE_DIR"
    echo "    /home/evgeny/.npm-global/bin/claude → $CLAUDE_PATH"
    echo "    -home-evgeny-Github → $CLAUDE_PROJECT_SLUG"
    echo "    6 → $TIMEZONE_HOUR"
    echo "    9:00 MSK → $TIMEZONE_DESC"
    echo "    /home/evgeny → $HOME_DIR"
else
    find "$TEMPLATE_DIR" -type f \( -name "*.md" -o -name "*.json" -o -name "*.sh" -o -name "*.plist" -o -name "*.yaml" -o -name "*.yml" \) | while IFS= read -r file; do
        sed_inplace \
            -e "s|дlitvegpt|$GITHUB_USER|g" \
            -e "s|/home/evgeny/Github|$WORKSPACE_DIR|g" \
            -e "s|/home/evgeny/.npm-global/bin/claude|$CLAUDE_PATH|g" \
            -e "s|-home-evgeny-Github|$CLAUDE_PROJECT_SLUG|g" \
            -e "s|6|$TIMEZONE_HOUR|g" \
            -e "s|9:00 MSK|$TIMEZONE_DESC|g" \
            -e "s|/home/evgeny|$HOME_DIR|g" \
            "$file"
    done
